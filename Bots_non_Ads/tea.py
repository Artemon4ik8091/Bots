...
def tea_message(message):
    ...
    try:
        ...
        if time_delta.seconds < 1800:
            bot.reply_to(message, f'Ты уже пил чай {time_delta.seconds//60} минут назад. \n'
                                              f'Чай будет готов через {(30 - time_delta.seconds//60)} минут. ☕️')
            return
    except:
        pass
    ...
    if (TEA_PRICE >= 0.5):
        if (ANSWER == 1):
            bot.reply_to(message, "Ты выпил поллитровую банку чая. Куда столько?")
        elif (ANSWER == 2):
            bot.reply_to(message, "Ты как это сделал? + 0.5 литров")
        elif (ANSWER == 3):
            bot.reply_to(message, "0.5 литров чая = 1000 очков спокойствия и мотивации")
    if (TEA_PRICE < 0.5 and TEA_PRICE != 0):
        if (ANSWER == 1):
            bot.reply_to(message, f'Ты выпил чашку чая объёмом ' + str(TEA_PRICE) + 'л. ☕️\nСделал как любишь, присладил немного ^_^')
        if (ANSWER == 2):
            bot.reply_to(message, f'Привет! Рад видеть тебя, вот твой чай, наслаждайся ^_^\nТы выпил чашку чая объёмом ' + str(TEA_PRICE) + 'л. ☕️')
        if (ANSWER == 3):
            bot.reply_to(message, f'Ты выпил чашку чая объёмом ' + str(TEA_PRICE) + 'л. ☕️\nТвой чай, наслаждайся ^_^')
