...
def tea_message(message):
    ...
    try:
        ...
        if time_delta.seconds < 1800:
            bot.reply_to(message, f'Ты уже пил кофе {time_delta.seconds//60} минут назад. \n'
                                              f'кофе будет готов через {(30 - time_delta.seconds//60)} минут. ☕️' + AD_TEXT)
            return
    except:
        pass
    ...
    if (TEA_PRICE >= 0.5):
        if (ANSWER == 1):
            bot.reply_to(message, "Ты выпил поллитровую банку кофе. Куда столько?" + AD_TEXT)
        elif (ANSWER == 2):
            bot.reply_to(message, "Сердечко скоро передаст привет... + 0.5 литра" + AD_TEXT)
        elif (ANSWER == 3):
            bot.reply_to(message, "+ 0.5 литра кофе и + 500 очков энергичности" + AD_TEXT)
    if (TEA_PRICE < 0.5 and TEA_PRICE != 0):
        if (ANSWER == 1):
            bot.reply_to(message, f'Ты выпил чашку кофе объёмом ' + str(TEA_PRICE) + 'л. ☕️\nСделал как любишь, присладил немного и добавил молочка ^_^' + AD_TEXT)
        if (ANSWER == 2):
            bot.reply_to(message, f'Привет! Рад видеть тебя, вот твой кофеёк, наслаждайся ^_^\nТы выпил чашку кофе объёмом ' + str(TEA_PRICE) + 'л. ☕️' + AD_TEXT)
        if (ANSWER == 3):
            bot.reply_to(message, f'Ты выпил чашку кофе объёмом ' + str(TEA_PRICE) + 'л. ☕️\nТвой кофе, наслаждайся ^_^' + AD_TEXT)
